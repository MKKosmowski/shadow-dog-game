...
export class Player {
    constructor(game) {
        ...
        this.energyTimer = 0;
        this.energyInterval = 100; // 100 -> run | 20 -> sit 
        this.rollingMinEnergy = 30;
        this.diveMinEnergy = 40;
        this.speed = 0;
        this.maxSpeed = 10;
    ...
    update(input, deltaTime) {
        ...
        if(this.energyTimer > this.energyInterval) {
            this.energy += this.energyModifier;
        ...
        this.game.enemies.forEach(enemy => {
            if(
                ...
                if(this.currentState === this.states[4] || this.currentState === this.states[5]) {
                    ...
                    this.energy += 5;
                    this.game.floatingMessages.push(new FloatingMessages('+5♦', enemy.x, enemy.y, 150, 80));
                } else {
                    this.setState(6, 0);
                    this.game.lives--;
                    if(this.game.lives <= 0) {
                        this.game.gameOver = true;
                        // if(this.game.score > this.game.bestScore) this.game.bestScore = this.game.score;
                    }
                }
            }
        });
